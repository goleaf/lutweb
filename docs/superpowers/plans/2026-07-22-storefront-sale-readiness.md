...
- The July 23 configuration audit restored 59 previously undocumented custom settings in `.env.example` and added a Pest contract that inventories all environment keys consumed by the checkout, custom LUT, legal, tester, wizard, PayPal, security, SEO, and storefront media configuration files.
- The July 23 production integrity audit recalculated every stored size and SHA-256: all 1,500 private package/CUBE/README files and all 7,200 public JPEG/WebP variants match their database records, with zero missing or mismatched files. Both opt-in real FFmpeg integration tests also pass with 59 assertions.
- Locked Composer dependencies and production npm dependencies report zero known advisories; npm was checked against the official registry because the server's configured mirror does not implement the audit endpoint.
- The July 23 test-isolation audit found that a cached production configuration could override `phpunit.xml` during a direct `php artisan test` run. The production catalog, users, orders, and stored media remained unchanged; the single failed heartbeat row and one testing-disk artifact were removed after a verified forensic backup. PHPUnit now forces a dedicated testing config-cache path, and `Tests\TestCase` refuses to start Feature tests unless the application is in `testing` with in-memory SQLite.
- Production SQLite now requires the fail-closed `DB_PRODUCTION_SQLITE_APPROVED` operator acknowledgement. Both committed environment templates default to `false`; this intentionally low-traffic deployment is the only environment where the flag is enabled, allowing `production:doctor` to distinguish an explicit decision from accidental SQLite use.
