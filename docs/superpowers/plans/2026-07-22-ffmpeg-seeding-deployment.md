...
Внешние launch-ограничения, которые не заполняются фиктивными данными: production SMTP, PayPal credentials и включение checkout, финальное юридическое/налоговое подтверждение. Реальный каталог из 300 LUT уже опубликован и проверен; SQLite используется намеренно для текущего малонагруженного запуска и остаётся единственным ожидаемым замечанием строгого production doctor.
